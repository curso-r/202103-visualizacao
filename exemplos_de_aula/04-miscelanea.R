...
# "dados/shp/municipios_todos.rds"

municipios <- readr::read_rds("dados/shp/municipios_todos.rds") %>% 
  filter(abbrev_state %in% c("RS", "PR", "SC")) %>% 
  inner_join(dados_idh_muni)
...
da <- tbl_regression(modelo)

# gtsummary:::print.gtsummary
# da$table_body
# cols_label("Variável" = "Characteristic")
